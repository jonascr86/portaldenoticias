...
function video_destaque_metabox($post) {
    ...
    <img style="<?php if (empty($id_video)) {
        echo 'display: none;';
    } else {
        echo 'display: block';
    } ?>" class="thumb" src="http://img.youtube.com/vi/<?php echo $id_video; ?>/0.jpg" alt="<?php echo $titulo_video; ?>" />
...
function video_destacado($destacado = false) {
    $values = get_post_custom($post->ID);
    $id_video = isset($values['id_video']) ? esc_attr($values['id_video'][0]) : '';
